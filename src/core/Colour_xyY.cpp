...
auto km::convert_to_xyY(const XYZ& col) -> xyY
{
    xyY result;
...
auto km::convert_to_xyY(const Illuminant& rw) -> xyY
{
    return convert_to_xyY(static_cast<XYZ>(rw));
}

auto km::convert_to_xyY(const Lab& col) -> xyY
{
    return convert_to_xyY(convert_to_XYZ(col));
}

auto km::convert_to_xyY(const LCHab& col) -> xyY
{
    return convert_to_xyY(convert_to_Lab(col));
}

auto km::convert_to_xyY(const Luv& col) -> xyY
{
    return convert_to_xyY(convert_to_XYZ(col));
}

auto km::convert_to_xyY(const LCHuv& col) -> xyY
{
    return convert_to_xyY(convert_to_Luv(col));
}

auto km::convert_to_xyY(const RGB& col) -> xyY
{
    return convert_to_xyY(convert_to_XYZ(col));
}